...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Edit',
        href: `/undangans/${props.undangan.id}/edit`,
    },
];
...
const form = useForm({
  ...
  description_mempelai_2: props.undangan.content?.description_mempelai_2 || '',
  story_1: props.undangan.content?.story_1 || '',
  story_2: props.undangan.content?.story_2 || '',
  story_3: props.undangan.content?.story_3 || '',
...
function initMap() {
  ...
  if (form.url_maps) {
    ...
    const match = form.url_maps.match(/q=(-?\d+\.\d+),(-?\d+\.\d+)/);
    if (match) {
      initialLocation = [parseFloat(match[1]), parseFloat(match[2])];
    }
  }

  // Default location (Indonesia) if no coordinates found
  initialLocation = initialLocation || [-6.200000, 106.816666];
  ...
  fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(mapSearch.value)}`)
    ...
    .then(data => {
      if (data && data.length > 0) {
        ...
        // Update tempat field with display name only if it was empty
        if (!form.tempat) {
    ...
    .catch(error => {
      ...
      Swal.fire({
        ...
        timerProgressBar: true,
        icon: 'error',
        title: 'Error saat mencari lokasi'
      });
    });
}
...
<template>
  <Head title="Edit Undangan Pernikahan" />
