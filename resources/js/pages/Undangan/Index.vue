...
const debouncedSearch = debounce(() => {
  router.get(route('undangans.index'), { search: search.value }, {
    preserveState: true,
    preserveScroll: true,
    replace: true
  });
...
function deleteUndangan(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      router.delete(route('undangans.destroy', id), {
        onSuccess: () => {
          Swal.fire(
            'Terhapus!',
        ...
        onError: () => {
          Swal.fire(
            'Error!',
            'Terjadi masalah saat menghapus undangan.',
            'error'
          );
        }
      });
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            <div class="flex justify-between items-center">
                ...
                <Link :href="route('undangans.create')">
                    ...
                    <table class="w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-800">
                            <tr>
                                ...
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Nama Pengantin</th>
                                ...
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Tempat</th>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-24">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                            <tr v-for="(item, index) in undangan.data" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-800">
                                ...
                                <td class="px-4 py-2 whitespace-nowrap text-sm">{{ item.waktu_acara }}</td>
                                <td class="px-4 py-2 text-sm max-w-[200px] truncate">{{ item.tempat }}</td>
                                <td class="px-4 py-2 whitespace-nowrap">
                                    <div class="flex space-x-1">
                                        <Link :href="route('undangans.edit', item.id)">
                            ...
                            <tr v-if="undangan.data.length === 0">
                ...
                <div class="px-6 py-4 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700">
                    <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                        <div class="flex items-center text-sm text-gray-500 dark:text-gray-400">
                            Menampilkan
                            <span class="font-medium mx-1">{{ undangan.from || 0 }}</span>
                            hingga
                            <span class="font-medium mx-1">{{ undangan.to || 0 }}</span>
                            dari
                            <span class="font-medium mx-1">{{ undangan.total }}</span>
                            hasil
                        </div>

                        <nav class="flex flex-wrap justify-center gap-2" aria-label="Pagination">
                            <template v-for="(link, key) in undangan.links" :key="key">
                                <template v-if="link.label !== '&hellip;'">
                                    <Link
                                        ...
                                        :preserve-state="true"
                                    >
                                        <span v-if="link.label === '&laquo; Previous'">Sebelumnya</span>
                                        <span v-else-if="link.label === 'Next &raquo;'">Berikutnya</span>
