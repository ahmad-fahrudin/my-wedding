...
const debouncedSearch = debounce(() => {
  router.get(route('tamus.index'), { search: search.value }, {
    preserveState: true,
    preserveScroll: true,
    replace: true
  });
...
function deleteTamu(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      router.delete(route('tamus.destroy', id), {
        onSuccess: () => {
          Swal.fire(
            'Deleted!',
            'Tamu has been deleted.',
            'success'
          );
        },
        onError: () => {
          Swal.fire(
            'Error!',
            'There was a problem deleting the tamu.',
            'error'
          );
        }
      });
                        ...
                        <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                            <tr v-for="(item, index) in tamu.data" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-800">
                                ...
                                <!-- <td class="px-4 py-2 whitespace-nowrap">
                                    <span
                                        :class="[
                                            'px-1.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full',
                                            item.status === 1
                                                ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300'
                                                : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300',
                                        ]"
                                    >
                                        {{ item.status === 1 ? 'Hadir' : 'Tidak Hadir' }}
                                    </span>
                                </td> -->
                                <td class="px-4 py-2 whitespace-nowrap">
                                    <div class="flex space-x-1">
                                        <Link :href="route('tamus.edit', item.id)">
